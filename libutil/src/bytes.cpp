...
optional<std::vector<uint8_t>> Util::Reader::read(int len) const {
...
bool Util::Writer::write(std::vector<uint8_t> const& data, int len) {
    if (len + m_offset > m_payload.size() * 8) {
        return false;
    }
    int bits_wrote = 0;
    while (bits_wrote < len) {
        ...
        int n = (bits_wrote / 8 >= data.size() ? 0 : data[bits_wrote / 8]) & mask;
