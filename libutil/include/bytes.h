...
public:
     ...
     * (len + 7) / 8
     */
    optional<std::vector<uint8_t>> read(int len) const;

    /**
     * @return true if end of data is reached
     */
    bool is_eof() const { return m_offset >= m_payload.size()*8; }
     ...
     */
    template <typename T>
    optional<T> read_int(int len) const {
        if (len > sizeof(T)*8) {
            return {};
        }
...
public:
     ...
     * (len + 7) / 8
     */
    virtual bool write(std::vector<uint8_t> const& data, int len);
...
public:
     ...
     */
    bool write(std::vector<uint8_t> const& data, int size) override {
