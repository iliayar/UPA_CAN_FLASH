...
struct VarField {
     ...
     */
    VarField(T value, int size) noexcept : m_size(size), m_value(value), m_valid(true) {}
    VarField(int size) noexcept : m_size(size), m_valid(false) {}
    VarField() noexcept : m_size(0), m_valid(false) {}
     ...
     * in deriving classes through method {@link #read_impl(Reader const&)}. If read
...
protected:
    virtual optional<T> read_impl(Reader const& reader) = 0;
...
struct Field : VarField<T> {
    Field(T value) noexcept : VarField<T>(value, size) {}
    Field() noexcept : VarField<T>(size) {}
};
 ...
 */
template <typename T, int size>
struct IntField : Field<T, size> {
    IntField(T value) noexcept : Field<T, size>(value) {}
    IntField() noexcept : Field<T, size>() {}

protected:
    bool write_impl(Writer& writer, T value) {
        return writer.write_int<T>(value, size);
    }
    optional<T> read_impl(Reader const& reader) { return reader.read_int<T>(size); }
};

/**
 * Field for an arbitrary number of bytes. Resizable
 */
struct VarVecField : VarField<std::vector<uint8_t>> {
    VarVecField(std::vector<uint8_t> data, int size) noexcept
        : VarField<std::vector<uint8_t>>(data, size) {}
    VarVecField(int size) noexcept : VarField<std::vector<uint8_t>>(size) {}
    VarVecField() noexcept : VarField<std::vector<uint8_t>>() {}

    VarVecField& operator=(std::vector<uint8_t> const& value) {
        m_value = value;
        m_size = value.size()*8;
        m_valid = true;
        return *this;
...
protected:
    ...
    optional<std::vector<uint8_t>> read_impl(Reader const& reader) {
        if(m_all) {
 ...
 */
template <int size>
struct VecField : VarVecField {
    VecField(std::vector<uint8_t> data) noexcept : VarVecField(data, size) {}
    VecField() noexcept : VarVecField(size) {}
};
...
struct EnumField : Field<T, size> {
    EnumField(T value) noexcept : Field<T, size>(value) {}
    EnumField() noexcept : Field<T, size>() {}
...
protected:
    ...
    optional<T> read_impl(Reader const& reader) {
...
protected:
    ...
    optional<std::shared_ptr<T>> read_impl(Reader const& reader) {
...
template <typename... F>
bool read_args(Reader const& reader, F&... args) {
    bool res = true;
    over_all<F&...>::for_each(
        [&](auto& field) {
            field.read(reader);
...
protected:
     ...
     */
    template <typename... F>
    void read(Reader const& reader, F&... fields) {
