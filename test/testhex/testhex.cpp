...
");
    ...
    EXPECT_EQ(static_cast<Hex::ExtendLinearAddressLine*>(line)->get_address(),
	      0x0800);

    line = reader.read_line();
    EXPECT_EQ(line->get_type(), Hex::HexLineType::Data);
    EXPECT_EQ(static_cast<Hex::DataLine*>(line)->get_address(), 0x0000);
    EXPECT_EQ(
	static_cast<Hex::DataLine*>(line)->get_data(),
	std::vector<uint8_t>({0x50, 0x22, 0x00, 0x20, 0x69, 0x47, 0x01, 0x08,
			      0x71, 0x47, 0x01, 0x08, 0x73, 0x47, 0x01, 0x08}));

    line = reader.read_line();
    EXPECT_EQ(line->get_type(), Hex::HexLineType::Data);
    EXPECT_EQ(static_cast<Hex::DataLine*>(line)->get_address(), 0xfff0);
    EXPECT_EQ(
	static_cast<Hex::DataLine*>(line)->get_data(),
    ...
    EXPECT_EQ(static_cast<Hex::StartLinearAddressLine*>(line)->get_address(),
	      0x080000ed);
